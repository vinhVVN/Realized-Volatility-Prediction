...
class NeighborFeatures:
    ...
    def __init__(self, metric: str = 'minkowski', p: int = 1, n_neighbors_max: int = 80):
        ...
        # Tận dụng tối đa CPU (n_jobs=-1) để tìm neighbors siêu tốc
        self.knn = NearestNeighbors(n_neighbors=n_neighbors_max, metric=metric, p=p, n_jobs=-1)
...
class TimeIdNeighbors(NeighborFeatures):
    ...
    def generate_features(self, df: pd.DataFrame, pivot_col: str, target_cols: List[str], 
        ...
        # Điền missing values bằng giá trị trung bình của mỗi cột (stock)
        ...
        # 2. Fit KNN để lấy chỉ số top 80 neighbors
        self.knn.fit(scaled_values)
        _, self.neighbor_indices = self.knn.kneighbors(scaled_values)
        
        # Khởi tạo dataframe kết quả chỉ gồm time_id và stock_id (để dễ merge)
        result_df = df[['time_id', 'stock_id']].copy()
        
        # 3. Aggregate cho từng target feature với mảng tốc độ cao
        for target_col in target_cols:
            # Pivot target feature thành ma trận (N_time_ids, N_stock_ids)
            target_pivot = df.pivot(index='time_id', columns='stock_id', values=target_col).values
            
            for n in n_windows:
                # Lấy N neighbors đầu tiên
                k_indices = self.neighbor_indices[:, :n]
                
                # Trích xuất values từ neighbors: shape (N_time_ids, n, N_stock_ids)
                neighbor_values = target_pivot[k_indices]
                
                # Tính toán numpy vectorization dọc theo axis 1 (n neighbors)
                ...
                # Stack ma trận lại thành định dạng dataframe ban đầu
        ...
        # 4. Tính toán relative rank (vị trí tương đối so với neighbor)
        # Sử dụng pd.merge để lấy chính xác giá trị gốc tránh trường hợp sai lệch order
        ...
        # Xóa các cột target_col gốc đã mượn
        result_df.drop(columns=target_cols, inplace=True)
        
        # Set index chuẩn để dễ join với dataset chính bên ngoài
...
class StockIdNeighbors(NeighborFeatures):
    ...
    Logic: Gom nhóm các cổ phiếu có hành vi giá hoặc biến động giống nhau (thường là cùng ngành/sector).
    """
    def generate_features(self, df: pd.DataFrame, pivot_col: str, target_cols: List[str], 
                          n_windows: List[int], prefix: str) -> pd.DataFrame:
        """
        Tạo NN features bằng cách nhóm theo cổ phiếu.
        """
        ...
        # 3. Aggregate
        for target_col in target_cols:
            # Pivot target: shape (N_stock_ids, N_time_ids)
            ...
            for n in n_windows:
                ...
                # Trích xuất values: shape (N_stock_ids, n, N_time_ids)
                neighbor_values = target_pivot[k_indices]
                
                # Tính toán dọc theo trục neighbors
                mean_val = np.nanmean(neighbor_values, axis=1)
                min_val = np.nanmin(neighbor_values, axis=1)
                max_val = np.nanmax(neighbor_values, axis=1)
                std_val = np.nanstd(neighbor_values, axis=1)
                ...
                def stack_matrix(mat, feat_name):
                    # mat shape: (N_stock_ids, N_time_ids)
